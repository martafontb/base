...
class CartDrawer {
    ...
    renderCart() {
      ...
      if (window.cart.item_count === 0) {
        ...
        const itemsHTML = window.cart.items.map(item => this.renderCartItem(item)).join('');
          ...
          <form action="/cart" method="post" id="CartDrawerForm" class="cart-form">
            ...
            <div class="cart-footer">
              <div class="cart-subtotal">
                ...
                <span id="CartDrawerSubtotal">${formatMoney(window.cart.total_price)}</span>
              </div>
              
              <p class="cart-disclaimer">${window.theme?.strings?.taxes_and_shipping_at_checkout || 'Taxes and shipping calculated at checkout'}</p>
    ...
    renderCartItem(item) {
      ...
      if (item.options_with_values && item.options_with_values.length > 0) {
        optionsHTML += item.options_with_values.map(option => `
      ...
      return `
        <div class="cart-item" data-id="${item.key}">
          ...
          <div class="cart-item__content">
            ...
            ${optionsHTML ? `<div class="cart-item__options">${optionsHTML}</div>` : ''}
  ...
  document.addEventListener('shopify:section:load', (event) => {
    if (event.target.querySelector('cart-drawer') || event.target.querySelector('cart-toggle')) {
      new CartDrawer();
    }
  });